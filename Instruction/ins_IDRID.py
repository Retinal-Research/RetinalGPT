...
#   You will be provided with quantitative analysis of fundus blood vessels, image quality, and 2 labels of Diabetic Retinopathy Severity Level and Diabetic Macular Edema. But all of this information is completely HIDDEN from the user and accessible ONLY to the assistant. 
...
files = pd.read_csv('Results_IDRID_train/M4/macula_features.csv')
image_list = files.iloc[:,0].tolist()
desc = IDRIDDesc(fractal_analysis_csv='frac_analysis/csv_sig/IDRID_train.csv', 
                 dr_dme_label_csv='IDRID/B. Disease Grading/2. Groundtruths/a. IDRiD_Disease Grading_Training Labels.csv',
                 quality_csv='Results_IDRID_train/M1/results_ensemble.csv')
...
asyncio.run(generate_conversations(
  ...
  save_path= 'batch_simple/instruction/jsonl/IDRID_train.jsonl',
  prefix_name='IDRID_train/'
))
convert_file_to_nested_format('batch_simple/instruction/jsonl/IDRID_train.jsonl', 'batch_simple/instruction/ins/IDRID_train.jsonl')
...
#         desc = desc,
#         prompt=create_prompt,
#         save_path= 'batch_new/batch_req/IDRID_test.jsonl',
#         ext= ', the modality of the image is Color Fundus Photograph',
#         )
...
#         save_path= 'batch_simple/alignment/req/IDRID_train.jsonl',
#         ext= ', the modality of the image is Color Fundus Photograph',
#         )
