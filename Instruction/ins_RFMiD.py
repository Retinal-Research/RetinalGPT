...
#   You are provided with quantitative analysis of fundus blood vessels, image quality, and a disease information. All of this information is completely HIDDEN from the user and accessible ONLY to the assistant.
...
files = pd.read_csv('RFMiD/1. Original Images/Results_train/M4/macula_features.csv')
image_list = files.iloc[:,0].tolist()
desc = RFMiDDesc(fractal_analysis_csv='frac_analysis/csv_sig/RFMiD_train.csv', 
                 quality_csv='RFMiD/1. Original Images/Results_train/M1/results_ensemble.csv', 
                 disease_csv='RFMiD/2. Groundtruths/a. RFMiD_Training_Labels.csv')

# asyncio.run(generate_conversations(
#   image_list=image_list[600:],
#   prompt=create_prompt,
#   desc=desc,
#   save_path= 'batch_simple/instruction/jsonl/RFMiD_val.jsonl',
#   prefix_name='RFMiD_val/',
#   ext=', the modality of the image is Color Fundus Photograph'

# ))
# convert_file_to_nested_format('batch_simple/instruction/jsonl/RFMiD_val.jsonl', 'batch_simple/instruction/ins/RFMiD_val.json')
...
def create_batch_file(img_list, desc):
    ...
    create_batch(
        ...
        desc = desc,
        prompt=create_prompt,
        save_path= 'batch_simple/instruction/req/RFMiD_train1.jsonl',
        ext=', the modality of the image is Color Fundus Photograph'
        )
...
# files = pd.read_csv('RFMiD/1. Original Images/Results_val/M4/macula_features.csv')
# image_list = files.iloc[:,0].tolist()
# desc = RFMiDDesc(
#                  quality_csv='RFMiD/1. Original Images/Results_val/M1/results_ensemble.csv', 
#                  disease_csv='RFMiD/2. Groundtruths/b. RFMiD_Validation_Labels.csv')

# asyncio.run(generate_conversations(
#   image_list=image_list[15:20],
#   prompt=create_prompt,
#   desc=desc,
...
#         save_path= 'batch_simple/alignment/req/RFMiD_val.jsonl',
#         ext=', the modality of the image is Color Fundus Photograph'
#         )
